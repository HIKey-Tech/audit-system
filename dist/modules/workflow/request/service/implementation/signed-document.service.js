...
exports.signedDocumentService = exports.SignedDocumentService = void 0;
...
const generationInclude = client_1.Prisma.validator()({
...
class SignedDocumentService {
    ...
    async generateForCompletedRequest(requestId) {
        try {
            ...
            const manifestHash = [...request.actions].reverse().find((a) => a.action_type === 'sign')?.signature_hash ?? '—';
            const attachments = (await this.documents.listByEntity(ATTACHMENT_ENTITY_TYPE, requestId)).filter((d) => d.mimeType === 'application/pdf');
            if (attachments.length === 0) {
                const bytes = await (0, signed_document_utility_1.buildCertificatePdf)({
            ...
            for (const att of attachments) {
                try {
                    ...
                    const data = {
                        ...
                        title: request.title,
                        manifestHash,
                        fileChecksum: crypto_1.default.createHash('sha256').update(file.buffer).digest('hex'),
                        entries,
                    };
                    const signed = await (0, signed_document_utility_1.appendSignaturePage)(file.buffer, data);
                    ...
                    logger_util_1.logger.warn('Signed-PDF generation failed for attachment', {
                        requestId,
                        documentId: att.id,
                        err,
                    });
    ...
    async list(requestId) {
        const rows = await prisma_client_1.prisma.workflow_Request_Signed_Document.findMany({
    ...
    async _buildEntries(request) {
        ...
        for (const step of steps) {
            if (step.status !== 'signed' && step.status !== 'approved')
                continue;
            ...
            if (step.status === 'signed') {
                ...
                if (action?.signature_id) {
                    const sig = await prisma_client_1.prisma.user_Signature.findUnique({ where: { id: action.signature_id } });
                    if (sig) {
    ...
    async _store(requestId, sourceId, baseName, bytes, uploadedById) {
        ...
        await prisma_client_1.prisma.workflow_Request_Signed_Document.create({
            data: { request_id: requestId, source_document_id: sourceId, signed_document_id: doc.id },
        });
        logger_util_1.logger.info('Signed document generated', { requestId, sourceId, signedDocumentId: doc.id });
    }
}
exports.SignedDocumentService = SignedDocumentService;
exports.signedDocumentService = new SignedDocumentService(new document_1.DocumentService());
//# sourceMappingURL=signed-document.service.js.map