...
class DashboardController {
    ...
    _registerRoutes() {
         ...
         */
        this.router.get('/summary', (0, auth_middleware_1.requirePermission)('dashboard:read'), this._getAuditSummary.bind(this));
        /**
         * @route  GET /dashboard/findings
         * @desc   Findings summary (severity / status / resolution metrics)
         * @access Private — finding:read
         */
        this.router.get('/findings', (0, auth_middleware_1.requirePermission)('dashboard:read'), this._getFindingsSummary.bind(this));
        /**
         ...
         * @access Private — audit:read
         */
        this.router.get('/activity', (0, auth_middleware_1.requirePermission)('dashboard:read'), (0, validate_middleware_1.validate)(dashboard_request_dto_1.ActivityQuerySchema, 'query'), this._getRecentActivity.bind(this));
        /**
         * @route  GET /dashboard/escalations
         * @desc   Active escalations overview (level / recent list)
         * @access Private — audit:read
         */
        this.router.get('/escalations', (0, auth_middleware_1.requirePermission)('dashboard:read'), this._getEscalationOverview.bind(this));
        /**
         * @route  GET /dashboard/my-work
         * @desc   Personal work bundle for the current user
         * @access Private — audit:read
         */
        this.router.get('/my-work', (0, auth_middleware_1.requirePermission)('dashboard:read'), this._getMyWork.bind(this));
        /**
         * @route  GET /dashboard/approval-inbox
         * @desc   Personal approval inbox summary for the current user
         * @access Private — audit:read
         */
        this.router.get('/approval-inbox', (0, auth_middleware_1.requirePermission)('dashboard:read'), this._getApprovalInboxSummary.bind(this));
        /**
    ...
    async _getAuditAnalytics(req, res, next) {
        try {
            const analytics = await this.dashboardService.getAuditAnalytics(req.user);
            res.status(200).json((0, api_response_type_1.buildResponse)(analytics));
        }
        catch (err) {
            next(err);
        }
